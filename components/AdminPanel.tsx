...
function GroupsTab() {
  ...
  const openCreate = () => {
    ...
    setMemberSearch("");
    setSelectedMembers([]);
    api("/users").then(r => r.ok ? r.json() : []).then(d => setAvailableUsers(Array.isArray(d) ? d : [])).catch(() => {});
  };
  ...
  const handleCreateGroup = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    try {
      const res = await api("/groups", { method: "POST", body: JSON.stringify({ name: groupName.trim(), memberIds: selectedMembers }) });
      const data = await res.json();
  ...
  const filteredUsers = (availableUsers || []).filter(u =>
    !memberSearch.trim() || (u.name || u.username || "").toLowerCase().includes(memberSearch.toLowerCase()) || u.email.toLowerCase().includes(memberSearch.toLowerCase())
  );
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="bg-canvas border border-hairline rounded-sm">
        <div className="px-5 py-4 border-b border-hairline flex items-center justify-between">
          ...
          <button onClick={openCreate} className="bg-primary hover:bg-primary-focus text-white caption-strong px-4 py-2 rounded-xs flex items-center gap-1.5 transition-all active:scale-95 cursor-pointer">
            <UserPlus className="w-3.5 h-3.5" /> Add Group
          </button>
        </div>
      ...
      <Modal open={showCreate} onClose={() => { if (!creating) setShowCreate(false); }} title="Add Chat Group">
        <form onSubmit={handleCreateGroup} className="space-y-5">
          {groupError && (
            <div className="bg-red-500/10 border border-red-500/20 text-red-500 caption font-bold px-4 py-3 rounded-xs flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 shrink-0" /> {groupError}
            </div>
          )}

          <div>
            <label className="block text-ink-muted-48 mb-1.5 font-bold uppercase tracking-wider fine-print">Group Name *</label>
            <input type="text" value={groupName} onChange={e => setGroupName(e.target.value)} placeholder="e.g. Marketing Team" autoFocus
              className="w-full bg-canvas-parchment border border-hairline p-3 rounded-xs caption text-ink placeholder:text-ink-muted-48 outline-none focus:border-primary transition-all" />
          </div>

          <div>
            <label className="block text-ink-muted-48 mb-1.5 font-bold uppercase tracking-wider fine-print">Members</label>
            <div className="relative mb-2">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-ink-muted-48" />
              <input type="text" value={memberSearch} onChange={e => setMemberSearch(e.target.value)} placeholder="Search users..."
                className="w-full bg-canvas-parchment border border-hairline pl-9 pr-3 py-2.5 rounded-xs caption text-ink placeholder:text-ink-muted-48 outline-none focus:border-primary transition-all" />
            </div>
            <div className="flex flex-wrap gap-2 mb-2">
              {selectedMembers.length > 0 && availableUsers.filter(u => selectedMembers.includes(u.id)).map(u => (
                <span key={u.id} className="flex items-center gap-1.5 bg-primary/10 border border-primary/20 text-primary caption-strong px-2.5 py-1 rounded-xs">
                  {(u.name || u.username)}
                  <button type="button" onClick={() => setSelectedMembers(p => p.filter(id => id !== u.id))} className="text-primary hover:text-red-500 transition-colors cursor-pointer">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
            </div>
            <div className="max-h-40 overflow-y-auto border border-hairline rounded-xs custom-scrollbar">
              {filteredUsers.length === 0 ? (
                <p className="caption text-ink-muted-48 text-center py-6">No users found</p>
              ) : (
                filteredUsers.map(u => {
                  const selected = selectedMembers.includes(u.id);
                  return (
                    <button type="button" key={u.id} onClick={() => setSelectedMembers(p => selected ? p.filter(id => id !== u.id) : [...p, u.id])}
                      className={`w-full flex items-center gap-3 px-3 py-2.5 text-left transition-all cursor-pointer ${selected ? "bg-primary/5" : "hover:bg-canvas-parchment"}`}>
                      <div className={`w-5 h-5 rounded-xs border flex items-center justify-center shrink-0 transition-all ${selected ? "bg-primary border-primary" : "border-hairline"}`}>
                        {selected && <CheckCircle className="w-3.5 h-3.5 text-white" />}
                      </div>
                      <span className="caption-strong text-ink">{u.name || u.username}</span>
                      <span className="caption text-ink-muted-48 ml-auto">{u.role}</span>
                    </button>
                  );
                })
              )}
            </div>
            <p className="fine-print text-ink-muted-48 mt-1.5">{selectedMembers.length} member{selectedMembers.length !== 1 ? "s" : ""} selected</p>
          </div>
